...
class TestExtractCardNumber:
    ...
    code = '123456789123'
    url = "/api/v1/extract-card-number"

    @pytest.fixture()
    def test_image(self) -> TypeImage:
        size = (180, 650)
        list_text = ["Serial Number:", f"0000123456789            {self.code}"]
    ...
    def test_extract_card_number_with_valid_image(self, test_image: TypeImage) -> None:
        ...
        assert response.json() == {"cardNumbers": [self.code], "status": "success", "message": "card number extracted successfully"}
